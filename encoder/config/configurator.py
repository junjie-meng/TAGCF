...
def parse_configure(model=None, dataset=None):
    ...
    parser.add_argument('--keep_rate', type=float, default=None, help='Override model keep_rate when explicitly set')
    ...
    with open('./encoder/config/modelconf/{}.yml'.format(model_name), encoding='utf-8') as f:
        ...
        if args.keep_rate is not None:
            configs['model']['keep_rate'] = args.keep_rate
            if configs['data']['name'] in configs['model']:
                configs['model'][configs['data']['name']]['keep_rate'] = args.keep_rate
